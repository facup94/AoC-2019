...
func runPruebas() {
	...
	instructions := make([]int, len(entrada))
	for index := 0; index < len(entrada); index++ {
		instructions[index], _ = strconv.Atoi(entrada[index])
	}

	createFile("/Users/fparra/go/src/github.com/facup94/AoC2019-go/output2.txt")
...
func software(instructions []int) int {
	...
	for programCounter > -1 {
		if programCounter == 0 {
			writeFile("/Users/fparra/go/src/github.com/facup94/AoC2019-go/output2.txt", fmt.Sprintf("%v %v\n", programCounter, instructions))
		}
